...
export async function sendOtpEmail(to: string, otp: string): Promise<void> {
  const from = process.env.EMAIL_FROM || `AgriSol <${process.env.EMAIL_USER || 'noreply@agrisol.com'}>`;

  // Print OTP to backend console for instant local development/testing visibility
  console.log(`=========================================`);
  console.log(`🔑 [AGRISOL OTP CODE] For: ${to}`);
  console.log(`✨ CODE: ${otp}`);
  console.log(`=========================================`);

  if (!process.env.EMAIL_USER || process.env.EMAIL_USER.includes('your_gmail')) {
    console.log(`ℹ️ SMTP credentials unconfigured in backend/.env — OTP logged above for local testing.`);
    return;
  }
  ...
  try {
    ...
    console.warn('⚠️ SMTP Email Delivery Warning:', err.message);
    console.warn('ℹ️ You can still use the logged OTP code above to complete verification locally!');
  }
}
